...
router.get('/', async (req, res) => {
  if (req.user && req.user.userRole === 1) {
    ...
    result ? res.status(200).send({ status: 'Ok', data: result }) : res.status(500).send({ status: 'FAILED', statusCode: 500 })
  } else {
...
router.post('/', async (req, res) => {
  if (req.user && req.user.userRole === 1) {
    if (req.body) {
      ...
      result ? res.status(200).send({ status: 'OK', message: 'Routes inserted' }) : res.status(500).send({ status: 'FAILED', statusCode: 500 })
    } else {
...
router.delete('/:idRoute', async (req, res) => {
  if (req.user.userRole === 1) {
    ...
    res ? res.status(200).send({ status: 'OK', message: 'Route deleted' }) : res.status(404).send({ status: 'FAILED', message: 'ROUTE NOT FOUND' })
  } else {
    res.status(401).send({ status: 'FORBIDDEN' })
  }
})
